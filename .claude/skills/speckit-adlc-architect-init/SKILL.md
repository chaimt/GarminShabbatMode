---
name: speckit-adlc-architect-init
...
metadata:
  ...
  source: architect:commands/init.md
user-invocable: true
...
1. **Run Setup Script**:
   - Execute `.specify/scripts/bash/setup-architect.sh "init $ARGUMENTS"` to initialize architecture files
...
1. Run `.specify/scripts/bash/setup-architect.sh "init $ARGUMENTS"` which calls `scan_existing_docs()`
