...
5. **Check Existing Documentation**:
   ...
   - Check team directives: Run `.specify/scripts/bash/setup-architect.sh "specify $ARGUMENTS"` and look for `TEAM_AGENTS_MD` in output - if present, this file contains usage instructions for team-wide agent directives
...
1. **Run Setup Script**:
   - Execute `.specify/scripts/bash/setup-architect.sh "specify $ARGUMENTS"` to ensure `{REPO_ROOT}/.specify/drafts/adr.md` exists
